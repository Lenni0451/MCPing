...
/**
 * The ping implementation for the bedrock edition (using raknet (old)).<br>
 * Ping response: {@link BedrockRaknetPingResponse}
 */
public class BedrockRaknetPing extends AUDPPing {

    private static final byte[] RAKNET_UNCONNECTED_MAGIC = new byte[]{0, -1, -1, 0, -2, -2, -2, -2, -3, -3, -3, -3, 18, 52, 86, 120};

    private final Random rnd = new Random();

    public BedrockRaknetPing(final IUDPSocketFactory socketFactory, final int readTimeout) {
    ...
    public void ping(ServerAddress serverAddress, IStatusListener statusListener) {
        try (IUDPSocket s = this.connect(serverAddress)) {
            ...
            this.readPacket(s, 32767, packetIs -> {
                ...
                BedrockRaknetPingResponse pingResponse = this.gson.fromJson(response, this.getResponseClass());
    ...
    // TODO: Remove this once BedrockPing is removed (backwards compatibility)
    protected Class<? extends BedrockRaknetPingResponse> getResponseClass() {
        return BedrockRaknetPingResponse.class;
    }
